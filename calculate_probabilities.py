...
def calculate_probabilities (StepwiseProbabilityObject, exp_df,CRITICAL_ILLNESSES):
    ...
    pZZ_calculated=pZZ.subs(sigma_R,StepwiseProbabilityObject.sol[0]).subs(sigma_SU,StepwiseProbabilityObject.sol[1]).subs(sigma_MU,StepwiseProbabilityObject.sol[2])
    ...
    pZZ_calculated_df_data=pd.DataFrame(data={"pii":'',"pZi":'',"pZZ":pZZ_calculated}, index=['pZZ'])
    ...
    #select the probability value from exp_df_initial that mathches the illness at location other than (~) empty (eq(''))
        ...
        pZi_calculated=pZi.subs(sigma_R,StepwiseProbabilityObject.sol[0]).subs(sigma_SU,StepwiseProbabilityObject.sol[1]).subs(sigma_MU,StepwiseProbabilityObject.sol[2])
        #temporary dataframe to enable concatenation
        pZi_calculated_df_data=pd.DataFrame(data={"pii":'',"pZi":pZi_calculated,"pZZ":''}, index=[illness])
        ...
        pii_calculated_df_data=pd.DataFrame(data={"pii":pii,"pZi":'',"pZZ":''}, index=[illness])
