...
def define_transitional_probabilities_functions_gamma (mortality_params_df_values, age, time, extramort):
    #defining symbols
    beta_1_Z,beta_2_Z,beta_1_i,beta_2_i,x,t,sigma_R,sigma_SU,sigma_MU,sigma_i,gamma=sm.symbols("beta_1_Z,beta_2_Z,beta_1_i,beta_2_i,x,t,sigma_R,sigma_SU,sigma_MU,sigma_i,gamma")
    ...
    pii=sm.exp(-sm.exp(beta_1_Z)/beta_2_Z*(1+gamma)*(sm.exp(beta_2_Z*(x+t))-sm.exp(beta_2_Z*x))-sm.exp(beta_1_i)/beta_2_i*(sm.exp(beta_2_i*(x+t))-sm.exp(beta_2_i*x)))
    #the probabilty of becomming ill from sickness i starting from age x till x+t when healty
    pZi=sm.exp(sm.exp(beta_1_Z)/beta_2_Z*sm.exp(beta_2_Z*x)*(1-sm.exp(beta_2_Z*t)*(1+gamma)+sm.exp(beta_1_Z)/beta_2_Z*gamma*sm.exp(beta_2_Z*(x+t/2))*(1-beta_2_Z*t/2)))*sm.exp(-sm.exp(beta_1_i)/beta_2_i*sm.exp(beta_2_i*(x+t))+sm.exp(beta_1_i)/beta_2_i*sm.exp(beta_2_i*(x+t/2))*(1-beta_2_i*t/2))*sigma_i/(-(sigma_R+sigma_SU+sigma_MU)+gamma*sm.exp(beta_1_Z)*sm.exp(beta_2_Z*(x+t/2))+sm.exp(beta_1_i)*sm.exp(beta_2_i*(x+t/2)))*sm.exp((sm.exp(beta_1_i)*sm.exp(beta_2_i*(x+t/2))+gamma*sm.exp(beta_1_Z)*sm.exp(beta_2_Z*(x+t/2))-(sigma_R+sigma_SU+sigma_MU))*t-1)
    ...
    #building an auxiliary list fo keep the expressiontypes, used later in DataFrame for to build a colum
    expressiontypes=[]
    ...
    for row in mortality_params_df_values.itertuples():
        if row[0]!="ZM": #means we are addressing one of the illness states
            ...
            transitional_probability_expressions_pZZ=pd.Series(data=pZZ.subs(beta_2_Z,mortality_params_df_values["beta2"][row[0]]).subs(beta_1_Z,mortality_params_df_values["beta1"][row[0]]).subs(x,age).subs(t,time).subs(gamma,extramort), index=[row[0]])
